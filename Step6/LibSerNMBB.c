...
void AfficheVoitureNMBB(struct VoitureNMBB *UnRecord)
{
	...
	MonPrintf(Tampon, 6, strlen(Tampon));
	sprintf(Tampon, "%s", UnRecord->Couleur);
	MonPrintf(Tampon, 30, strlen(Tampon));
	printf("\n");
}
...
{
	int value = 1, i = 0, ctrl = 0;
	FILE *fp;
	struct FactureNMBB temp;
	...
	else
	{
		while ((feof(fp) == 0) && (ctrl == 0))
		{
			fread(&temp, sizeof(struct FactureNMBB), 1, fp);

			if (temp.Reference == Reference && temp.DateFacturation == Date && (strcmp(NomClient, temp.NomClient) == 0))
			{
				value = 0;
				ctrl = 1;
			}
